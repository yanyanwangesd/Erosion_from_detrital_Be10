...
function production = CNP(DEM,ZONE,shielding_flag)
...
%       shielding_flag: flag (logical, 0 or 1) to determine whether
%                       shielding should be calculated to correct production rate. 0 is no
%                       shielding, 1 is yes.
% Output:
...
% Updated on March 7, 2023, to make shielding calculation optional, by
...
if shielding_flag~=0
    ...
    Tshd = DEM;
    Tshd.Z(~isnan(DEM.Z)) = 1; % no shielding so every pixel has correction coefficent 1
end
